...
> v1 대비 변경점: slug 네임스페이스 충돌 방지, tenant_id 격리 강제 수단 구체화(RLS 이중 방어),
...
| 계정 | 권한 | 용도 |
|---|---|---|
| `saas_app` | `tenant_saas.*` 에 DML만 (`SELECT/INSERT/UPDATE/DELETE`) | 애플리케이션 런타임 |
| `saas_migrate` | `tenant_saas.*` 에 `ALL` (DDL 포함) | Liquibase 마이그레이션 전용 |

- 런타임 계정은 **DDL 권한이 없다.** 애플리케이션이 스키마를 변경할 수 없다.
...
문자셋은 `utf8mb4` / `utf8mb4_unicode_ci`, 엔진은 `InnoDB`, `ROW_FORMAT=DYNAMIC`으로 고정한다.
마이그레이션 도구는 **Liquibase**를 쓴다 — Flyway Community Edition은 MySQL 5.7 지원을 중단해
Teams(유료) 라이선스를 요구하기 때문이다.
...
```text
PLATFORM_ADMIN   -- RLS 우회 롤, 전 테넌트 접근
...
```text
0. 공통 기반: TenantContext + Hibernate Filter + RLS + 격리 테스트   ← v2 최우선
...
> 나중에 전 테이블에 RLS를 소급 적용하는 대공사가 된다.
